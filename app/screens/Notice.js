...
class Notice extends Component {
  ...
  componentDidMount(){
      fetch('http://192.168.1.61/Aurorapp/notice.php')
      .then(res => res.json())
      .then(response => {
        this.setState({
          isLoading: false ,
  ...
  render() {
    ...
    if(this.state.isLoading == true) {
      return(
        <Container>
        <AppHeader  title="Notice to Decision" openDrawer={() => this.props.navigation.openDrawer()} />
        <Spinner  color = 'blue' />
      </Container>
      );
    }
      ...
      let newData = this.state.dataSource.map((val , key) => {
        return <CardList  openDetails = {() => this.props.navigation.navigate('NoticeDetails' , {
          username: val.username ,
          id: val.id ,
          isDraft: ""
        }) } fullName = {val.fullname} instance = {val.violation} dates = {val.dates}  key = {key} myKey = {key}  />
      });
