...
class EquipmentAccountability extends Component {
  ...
  componentDidMount(){
      fetch('http://192.168.1.61/Aurorapp/equipment_accountability.php')
      .then(res => res.json())
      .then(response => {
        this.setState({
          isLoading: false ,
  ...
  render() {
    ...
    if(this.state.isLoading == true) {
      return(
        <Container>
        <AppHeader  title="Equipment Accountability" openDrawer={() => this.props.navigation.openDrawer()} />
        <Spinner  color = 'blue' />
      </Container>
      );
    }
      ...
      let newData = this.state.dataSource.map((val , key) => {
        return <CardList  openDetails = {() => this.props.navigation.navigate('EquipmentDetails' , {
          username: val.username ,
          id: val.id ,
          isDraft: 0
        }) } fullName = {val.user_info.fullname} instance = {val.equipment} dates = {val.dates}  key = {key} myKey = {key}  />
      });
...
export default EquipmentAccountability;
