...
class IncidentReport extends Component {
  ...
  componentDidMount(){
      fetch('http://192.168.1.61/Aurorapp/incident_report.php')
      .then(res => res.json())
      .then(response => {
        this.setState({
          isLoading: false ,
  ...
  render() {
    ...
    if(this.state.isLoading == true) {
      return(
        <Container>
        <AppHeader  title="Incident Reports" openDrawer={() => this.props.navigation.openDrawer()} />
        <Spinner  color = 'blue' />
      </Container>
      );
    }
      ...
      let newData = this.state.dataSource.map((val , key) => {
        return <CardList  openDetails = {() => this.props.navigation.navigate('IncidentDetails' , {
          username: val.username ,
          id: val.id ,
          isDraft: 0
        }) } fullName = {val.fullname} instance = {val.paragraph} dates = {val.dates}  key = {key} myKey = {key}  />
      });
